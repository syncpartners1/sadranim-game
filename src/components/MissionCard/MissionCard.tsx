...
export const MissionCardComponent: React.FC<MissionCardProps> = ({
  ...
  return (
    <motion.div
      ...
      title={revealed ? `משימה סודית: ${mission.id}` : 'משימה סודית'}
    >
      {revealed ? (
        <img
...
export const MissionPile: React.FC<{ count: number; used: number }> = ({ count, used }) => (
  <div className="flex flex-col items-center gap-1">
    ...
    <span className="text-xs text-white/60">{count} משימות נותרו</span>
    <span className="text-xs text-white/40">{used} הושלמו</span>
  </div>
);
